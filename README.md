...
- 删除条文关键词
- 根据关键词自动匹配关联解析
- 用 SQLite（轻量数据库）存储运行时数据
- 保留 JSON（数据文件格式）文件作为最直接、最容易维护的数据来源
...
`平时先改 data（数据目录）里的 JSON（数据文件格式）或 TXT（文本文件），再根据需要导入数据库。`
...
打开某条条文 JSON（数据文件格式），把关键词写到 `keywords（关键词）` 数组里。
...
1. 把 TXT（文本文件）或 JSON（数据文件格式）放进 `data/关联解析/`
2. 在 `data/guanlianjiexiconfig.json` 里登记这个来源

### 4.5 添加新的经典

1. 准备经典原文
2. 生成或手工整理条文 JSON（数据文件格式）
...
`data/` 目录里的 JSON（数据文件格式）和 TXT（文本文件），是你平时最应该编辑的地方。
...
- 条文 JSON（数据文件格式）应该怎么写
- 关联解析 JSON（数据文件格式）应该怎么写
- TXT（文本文件）应该怎么放
- 新增经典的顺序是什么
- 数据库什么时候需要导入
...
项目现在已经支持打包成 Windows（微软视窗系统）桌面应用。
