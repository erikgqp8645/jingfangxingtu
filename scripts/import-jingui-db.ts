...
async function main() {
  ...
  const book = catalog.find(item => item.id === 'jingui');

  if (!book) {
    throw new Error('在 jingdianconfig.json 中未找到《金匮要略》配置');
  }
  ...
  try {
    ...
    db.prepare(`
      ...
      ON CONFLICT(id) DO UPDATE SET
        ...
        updated_at = excluded.updated_at
    `).run(book.id, book.name, 1, timestamp, timestamp);
    ...
    const stats = db.prepare(`
      SELECT
        ...
        (SELECT COUNT(*) FROM chapters WHERE book_id = 'jingui') AS chapters,
        (SELECT COUNT(*) FROM clauses WHERE book_id = 'jingui') AS clauses,
        (SELECT COUNT(*) FROM clause_keywords ck JOIN clauses c ON c.id = ck.clause_id WHERE c.book_id = 'jingui') AS clause_keywords
    `).get() as Record<string, number>;

    console.log(`Imported 《金匮要略》 into ${dbFile}`);
